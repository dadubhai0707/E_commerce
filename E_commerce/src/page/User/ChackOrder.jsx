    ...
    if (user && ordersState.length > 0) {
      const currentUserOrders = ordersState
        ...
        .flatMap(order => order.orders.map(ord => ({
          ...ord,
          order_id: order.order_id, // Attach order_id to each order
          states: order.states, // Attach states to each order
          time: order.time // Attach time to each order
        })));
                ...
                <div key={index} className="order-card">
                  ...
                  <div className="order-details">
                    <div className="name_Id">
                      <h3>{product.Pname}</h3>
                      <p><b>Order ID:</b> {order.order_id}</p> {/* Display order_id */}
                    </div>
                    <div className="order-date-status" style={{ margin: '10px 0' }}>
                      <div className="date">
                        ...
                        <p><b>Status:</b> {order.states || 'Status not available'}</p> {/* Display states */}
                      </div>
